...
class FrameTypeSystem(object):
    """ Simple type system hierarchy object, with associated methods.
    Used to navigate inheritance relations in FrameNet.
    """
    ...
    def build(self, fn):
        for frame in fn.frames:
            ...
            for root_frame in root_frames:
                ...
                if root_frame not in self.roots:
                    self.roots.append(root_frame)

    def get_ancestors(self, frame):
        """Returns all the ancestors for self."""
        fn = self.fn
        it = dfs_iterator(set(), lambda fr: fr.get_parents(fn), lambda fr: fr.ID)
        return it(frame)
    ...
    def toposort(self):
        """Returns all frames in tolopological order"""
        seen = set()
        fn = self.fn
        it = dfs_iterator(seen, lambda frame: frame.get_parents(fn), lambda frame: frame.ID)
        post = list(chain.from_iterable(it(f) for f in fn.frames if f.ID not in seen))
        # print(seen)
        assert len(post) == len(fn.frames), 'len(post) = %d != %d = len(fn.frames)' % (len(post), len(fn.frames))
